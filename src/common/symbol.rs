...
impl PartialEq for Symbol {
    fn eq(&self, other: &Self) -> bool {
        if self.is_eos() && other.is_eos() { true }
...
impl Symbol {
    pub fn new(token: Token) -> Self {
        Symbol::Identifier(SymbolIdentifier{token, identifier_type: "".to_string()})
    }
    ...
    pub fn change_type(&mut self, new_type: &str) {
        match self {
            Symbol::Identifier(t) => {t.identifier_type = new_type.to_string();}
            Symbol::EOS => {}
        }
    }
