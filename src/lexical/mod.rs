...
impl <'s>Scanner<'s> {
    ...
    pub fn init(&mut self) -> Result<Vec<Token>, Box<dyn Error + Send + Sync + 'static>> {
        ...
        loop {
            self.transition()?;
            if self.input.peek().is_none() { break; }
        }
...
impl <'s>Scanner<'s> {
    fn transition(&mut self) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
        ...
        match self.current_state {
            0 => {
                match current {
                    ...
                    c if c.is_ascii_digit() => {
                        ...
                        if next == '.' { self.current_state = 3; }
                            else if !next.is_ascii_digit() {
                                self.tokens.push(Token::new(&self.identifier_buffer, TokenType::Integer, self.line, self.column-self.identifier_buffer.len()+1));
                                self.identifier_buffer = String::new();
                                self.current_state = 0;
                            }
                    ...
                    c if c == '+' => {
                        self.tokens.push(Token::new("+", TokenType::AdditiveOperators, self.line, self.column));
                        self.current_state = 0;
                    }
                    c if c == '-' => {
                        self.tokens.push(Token::new("-", TokenType::AdditiveOperators, self.line, self.column));
                    ...
                    c if c == '*' => {
                        self.tokens.push(Token::new("*", TokenType::MultiplicativeOperators, self.line, self.column));
                        self.current_state = 0;
                    }
                    c if c == '/' => {
                        self.tokens.push(Token::new("/", TokenType::MultiplicativeOperators, self.line, self.column));
                    ...
                    c if c == '=' => {
                        self.tokens.push(Token::new("=", TokenType::RelationalOperators, self.line, self.column));
                        self.current_state = 0;
                    }
                    c if c == '>' => {
                        if next == '=' {
                            ...
                            self.tokens.push(Token::new(">", TokenType::RelationalOperators, self.line, self.column-1));
                            self.current_state = 0;
                        }
                    }
                    c if c == '<' => {
                    ...
                    c if c == ':' => {
                        if next == '=' {self.current_state = 14;}
                        else {
                            self.tokens.push(Token::new(":", TokenType::Delimiter, self.line, self.column));
                            self.current_state = 0;
                        }
                    }
                    c if c == ',' => {
            ...
            1 => {
                self.identifier_buffer.push(current);
                if !(current.is_alphanumeric() || current == '_') || !(next.is_alphanumeric() || next == '_') {
            ...
            3 => {
                ...
                if next.is_ascii_digit() {
                    ...
                    self.tokens.push(Token::new(&self.identifier_buffer, TokenType::Real, self.line, self.column+1-self.identifier_buffer.len()));
                    self.identifier_buffer = String::new();
                    self.current_state = 0;
                }
            },
            ...
            14 => {
                self.tokens.push(Token::new(":=", TokenType::Assignment, self.line, self.column-1));
                self.current_state = 0;
            },
            15 => {
